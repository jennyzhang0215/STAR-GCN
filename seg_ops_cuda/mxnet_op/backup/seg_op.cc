...
void SegMaxPoolBackwardImpl(const Tensor<cpu, 3, float> &dst,
  ...
  for (int k = 0; k < batch_num; k++) {
    for (int i = 0; i < seg_num; i++) {
      for (int c = 0; c < feat_dim; c++) {
        int target_ind = pool_indices[k][i][c];
        if (target_ind >= 0) {
          dst[k][target_ind][c] += ograd[k][i][c];
        }
      }
    }
  }
...
.set_attr<nnvm::FInferShape>("FInferShape", SegReduceShape)
...
.set_attr<nnvm::FInferShape>("FInferShape", SegBroadcastBinaryShape)
...
.set_attr<nnvm::FInferShape>("FInferShape", SegBroadcastToShape)
...
.set_attr<nnvm::FInferShape>("FInferShape", SegSoftmaxShape)
.set_attr<nnvm::FInferType>("FInferType", SegReduceSetType)
.set_attr<FResourceRequest>("FResourceRequest",
  [](const NodeAttrs& attrs) {
    return std::vector<ResourceRequest>{ResourceRequest::kTempSpace};
...
.set_attr<nnvm::FInferShape>("FInferShape", SegTakeKCorrShape)
...
.set_attr<nnvm::FInferShape>("FInferShape", SegWeightedPoolShape)
.set_attr<nnvm::FInferType>("FInferType", SegTakeKCorrSetType)
.set_attr<FResourceRequest>("FResourceRequest",
  [](const NodeAttrs& attrs) {
    return std::vector<ResourceRequest>{ResourceRequest::kTempSpace};
...
.set_attr<nnvm::FInferShape>("FInferShape", SegPoolShape)
...
.set_attr<nnvm::FGradient>("FGradient",
  [](const nnvm::NodePtr& n, const std::vector<nnvm::NodeEntry>& ograds) {
    ...
    nnvm::NodePtr p_data;
    if(param.pool_type == seg_op::SegReduceType::kMax) {
      p_data = MakeNode("_contrib__backward_seg_max_pool", n->attrs.name + "_backward_data", {ograds[0], nnvm::NodeEntry{n, 1, 0}}, nullptr, &n);
    } else {
