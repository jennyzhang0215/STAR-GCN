...
struct SegSoftmaxContigCUDA {
  static size_t get_temp_sum_bytes(int batch_num, int nnz, int seg_num) {
    ...
    SumOpCUB sum_op;
    cub::DeviceSegmentedReduce::Reduce<float*, float*>(NULL, temp_sum_storage_bytes, NULL, NULL,
      batch_num * seg_num, NULL, NULL,
  ...
  static size_t get_temp_max_bytes(int batch_num, int nnz, int seg_num) {
    ...
    MaxOpCUB max_op;
    cub::DeviceSegmentedReduce::Reduce<float*, float*>(NULL, temp_max_storage_bytes, NULL, NULL,
      batch_num * seg_num, NULL, NULL,
    ...
    for(int b_nid = UNROLL_NODE * TY_SZ * blockIdx.x; b_nid < node_num; b_nid += UNROLL_NODE * TY_SZ * gridDim.x) {
        ...
        for(int b_ind_inner = b_neighbor_ind; b_ind_inner < e_neighbor_ind; b_ind_inner += UNROLL_Y * TY_SZ) {
            ...
            if(threadIdx.y == 0) {
                ...
                for (int i = 0; i < CEIL_DIV(UNROLL_Y * TY_SZ, WARP_SZ); i++) {
                    if (b_ind_inner + i * WARP_SZ + threadIdx.x < e_ind_inner && i * WARP_SZ + threadIdx.x < UNROLL_Y * TY_SZ) {
                        atomicAdd(dst + k * nnz + b_ind_inner + i * WARP_SZ + threadIdx.x, dst_shared[i * WARP_SZ + threadIdx.x]);
                    }
                }
            }
            __syncthreads();
...
struct SegTakeKCorrCUDA {
    ...
    static void compute(float* dst, const float* embed1, const float* embed2,
        ...
        dim3 dimGrid(CEIL_DIV(node_num, UNROLL_NODE * TY_SZ), CEIL_DIV(feat_dim, WARP_SZ * UNROLL_X), K);
    ...
    static void compute_grad_embed2(float* dst, const float* g_out, const float* embed1,
        ...
        static const int WARP_SZ = 32;
        dim3 dimBlock(WARP_SZ, TY_SZ);
        dim3 dimGrid(CEIL_DIV(node_num, TY_SZ), CEIL_DIV(feat_dim, WARP_SZ * UNROLL_X), K);
        SegTakeKCorrBackwardEmbed2Kernel<TY_SZ, UNROLL_Y, UNROLL_X, WARP_SZ> <<<dimGrid, dimBlock, 0, stream>>>
            (dst, g_out, embed1, neighbor_ids, neighbor_ind_ptr, rev_node_ids, K, node_num, neighbor_node_num,
             nnz, feat_dim);
...
__global__ void
__launch_bounds__(WARP_SZ)
SegSumMeanPoolBackwardKernel(float* dst, const float* g_out, const int* indices, const int* indptr,
                                       int batch_num, int seg_num, int feat_dim, int total_ind_num, int nnz) {
    int batch_id = blockIdx.z;
    int c_begin = blockIdx.y * UNROLL_X * WARP_SZ;
    float g_out_local[UNROLL_X];
    for (int seg_id = blockIdx.x; seg_id < seg_num; seg_id += gridDim.x) {
        int ind_begin = indptr[seg_id];
        int ind_end = indptr[seg_id + 1];
        #pragma unroll
        for (int i = 0; i < UNROLL_X; i++) {
            int c = c_begin + i * WARP_SZ + threadIdx.x;
            if (c < feat_dim) {
                g_out_local[i] = g_out[IND3(batch_id, seg_id, c, seg_num, feat_dim)];
                if (reduce_type == SegReduceType::kMean && ind_end > ind_begin) {
                    g_out_local[i] /= (ind_end - ind_begin);
                }
            }
        }
        for (int j = ind_begin; j < ind_end; j++) {
            ...
            for(int i = 0; i < UNROLL_X; i++) {
                int c = c_begin + i * WARP_SZ + threadIdx.x;
                if(c < feat_dim) {
                    atomicAdd(dst + IND3(batch_id, indices[j], c, total_ind_num, feat_dim), g_out_local[i]);
                }
            }
        }
    }
...
/*Backward pass of the SegPool operator when max/min are used
...
for k = 0 to batch_num - 1
    for i = 0 to seg_num - 1
        for c = 0 to feat_num -1
            dst[k, out_index[k, i, c], c] += g_out[k, i, c]
*/
template<int UNROLL_X = 4, int WARP_SZ = 32>
__global__ void
__launch_bounds__(WARP_SZ)
SegMaxMinPoolBackwardKernel(float* dst, const float* g_out, const int* out_index,
                                      int batch_num, int seg_num, int feat_dim, int total_ind_num) {
    int batch_id = blockIdx.z;
    int c_begin = blockIdx.y * UNROLL_X * WARP_SZ;
    for (int seg_id = blockIdx.x; seg_id < seg_num; seg_id += gridDim.x) {
        #pragma unroll
        for(int i = 0; i < UNROLL_X; i++) {
            int c = c_begin + i * WARP_SZ + threadIdx.x;
            if(c < feat_dim) {
                int ind = IND3(batch_id, seg_id, c, seg_num, feat_dim);
                int dst_ind = out_index[ind];
                if(dst_ind >= 0) {
                    atomicAdd(dst + IND3(batch_id, dst_ind, c, total_ind_num, feat_dim), g_out[ind]);
                }
            }
        }
    }
...
struct SegPoolCUDA {
    ...
    static void compute_grad_data(float* dst, const float* g_out, const int* out_index, const int* indices, const int* indptr,
        ...
        dim3 dimGrid(seg_num, CEIL_DIV(feat_dim, WARP_SZ * UNROLL_X), batch_num);
        if(reduce_type == SegReduceType::kSum || reduce_type == SegReduceType::kMean) {
            SegSumMeanPoolBackwardKernel<reduce_type, UNROLL_X, WARP_SZ> <<<dimGrid, dimBlock, 0, stream>>> (dst, g_out, indices, indptr, batch_num, seg_num, feat_dim, total_ind_num, nnz);
            SEG_CUDA_POST_KERNEL_CHECK(SegSumMeanPoolBackwardKernel);
        } else {
            SegMaxMinPoolBackwardKernel<UNROLL_X, WARP_SZ> <<<dimGrid, dimBlock, 0, stream>>> (dst, g_out, out_index, batch_num, seg_num, feat_dim, total_ind_num);
            SEG_CUDA_POST_KERNEL_CHECK(SegMaxMinPoolBackwardKernel);
        }
    }
};
...
void SegSumMeanPoolBackwardImpl(const Tensor<gpu, 3, float> &dst,
  ...
  int nnz = indptr.shape_[0];
  int seg_num = ograd.shape_[1];
  cudaStream_t stream = Stream<gpu>::GetStream(s);
  if (req == kAddTo) {
    if(pool_type == SegReduceType::kSum) {
      SegPoolCUDA::compute_grad_data<SegReduceType::kSum, true>(dst.dptr_, ograd.dptr_, nullptr, indices.dptr_, indptr.dptr_,
                                                                batch_num, seg_num, feat_dim, total_ind_num, nnz, stream);
    } else if(pool_type == SegReduceType::kMean) {
      SegPoolCUDA::compute_grad_data<SegReduceType::kMean, true>(dst.dptr_, ograd.dptr_, nullptr, indices.dptr_, indptr.dptr_,
                                                                 batch_num, seg_num, feat_dim, total_ind_num, nnz, stream);
    } else {
    ...
    if(pool_type == SegReduceType::kSum) {
      SegPoolCUDA::compute_grad_data<SegReduceType::kSum, false>(dst.dptr_, ograd.dptr_, nullptr, indices.dptr_, indptr.dptr_,
                                                                 batch_num, seg_num, feat_dim, total_ind_num, nnz, stream);
    } else if(pool_type == SegReduceType::kMean) {
      SegPoolCUDA::compute_grad_data<SegReduceType::kMean, false>(dst.dptr_, ograd.dptr_, nullptr, indices.dptr_, indptr.dptr_,
                                                                  batch_num, seg_num, feat_dim, total_ind_num, nnz, stream);
    } else {
      LOG(FATAL) << "Not supported!";
    }
...
void SegMaxPoolBackwardImpl(const Tensor<gpu, 3, float> &dst,
                            const Tensor<gpu, 3, float> &ograd,
                            const Tensor<gpu, 3, int> &pool_indices,
                            const OpReqType req,
                            const OpContext &ctx,
  ...
  int total_ind_num = dst.shape_[1];
  int seg_num = ograd.shape_[1];
  int feat_dim = ograd.shape_[2];
  cudaStream_t stream = Stream<gpu>::GetStream(s);
  if (req == kAddTo) {
    SegPoolCUDA::compute_grad_data<SegReduceType::kMax, true>(dst.dptr_, ograd.dptr_, pool_indices.dptr_, nullptr, nullptr,
                                                              batch_num, seg_num, feat_dim, total_ind_num, -1, stream);
  } else {
    SegPoolCUDA::compute_grad_data<SegReduceType::kMax, false>(dst.dptr_, ograd.dptr_, pool_indices.dptr_, nullptr, nullptr,
                                                               batch_num, seg_num, feat_dim, total_ind_num, -1, stream);
  }
}
}  // namespace seg_op
