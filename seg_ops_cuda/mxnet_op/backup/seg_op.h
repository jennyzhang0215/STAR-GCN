...
void SegPoolImpl(const Tensor<gpu, 3, float> &dst_value,
                 ...
                 const OpContext &ctx,
                 Stream<gpu>* s);
// SegSumMeanPoolBackwardImpl
void SegSumMeanPoolBackwardImpl(const Tensor<cpu, 3, float> &dst,
...
void SegSumMeanPoolBackwardImpl(const Tensor<gpu, 3, float> &dst,
                                ...
                                const OpContext &ctx,
                                Stream<gpu>* s);
// SegSumMeanBackwardImpl
void SegMaxPoolBackwardImpl(const Tensor<cpu, 3, float> &dst,
                            const Tensor<cpu, 3, float> &ograd,
                            const Tensor<cpu, 3, int> &pool_indices,
                            const OpReqType req,
                            const OpContext &ctx,
...
inline bool SegReduceShape(const nnvm::NodeAttrs& attrs,
  ...
  using namespace mshadow;
  const TShape &data_shape = (*in_attrs)[0];
  const TShape &indptr_shape = (*in_attrs)[1];
...
inline bool SegBroadcastBinaryShape(const nnvm::NodeAttrs& attrs,
                                    ...
                                    std::vector<TShape> *out_attrs) {
  using namespace mshadow;
  const TShape &lhs_shape = (*in_attrs)[0];
  const TShape &rhs_shape = (*in_attrs)[1];
  const TShape &indptr_shape = (*in_attrs)[2];
...
inline bool SegTakeKCorrShape(const nnvm::NodeAttrs& attrs,
                              ...
                              std::vector<TShape> *out_attrs) {
  using namespace mshadow;
  const TShape &embed1_shape = (*in_attrs)[0];
  const TShape &embed2_shape = (*in_attrs)[1];
  const TShape &neighbor_ids_shape = (*in_attrs)[2];
  const TShape &neighbor_indptr_shape = (*in_attrs)[3];
...
inline bool SegWeightedPoolShape(const nnvm::NodeAttrs& attrs,
  ...
  using namespace mshadow;
  const TShape &data_shape = (*in_attrs)[0];
  const TShape &weights_shape = (*in_attrs)[1];
  const TShape &indices_shape = (*in_attrs)[2];
  const TShape &indptr_shape = (*in_attrs)[3];
...
inline bool SegPoolShape(const nnvm::NodeAttrs& attrs,
  ...
  const TShape &data_shape = (*in_attrs)[0];
  const TShape &indices_shape = (*in_attrs)[1];
  const TShape &indptr_shape = (*in_attrs)[2];
  if (data_shape.ndim() != 3) return false;
  if (indices_shape.ndim() != 1) return false;
  if (indptr_shape.ndim() != 1) return false;
...
void SegPoolForward(const nnvm::NodeAttrs& attrs,
  ...
  seg_op::SegPoolImpl(dst, pool_indices, data, indices, indptr, param.pool_type, req[0], ctx, s);
}
...
void SegSumMeanPoolBackward(const nnvm::NodeAttrs& attrs,
  ...
  Tensor<xpu, 1, int> indptr = inputs[2].get<xpu, 1, int>();
  Tensor<xpu, 3, float> dst = outputs[0].get<xpu, 3, float>();
  CHECK(param.pool_type == seg_op::SegReduceType::kMean || param.pool_type == seg_op::SegReduceType::kSum)
    << "Only support mean and sum in the backward";
  seg_op::SegSumMeanPoolBackwardImpl(dst, ograd, indices, indptr, param.pool_type, req[0], ctx, s);
}
...
void SegMaxPoolBackward(const nnvm::NodeAttrs& attrs,
  ...
  Stream<xpu> *s = ctx.get_stream<xpu>();
  Tensor<xpu, 3, float> ograd = inputs[0].get<xpu, 3, float>();
  Tensor<xpu, 3, int> pool_indices = inputs[1].get<xpu, 3, int>();
  Tensor<xpu, 3, float> dst = outputs[0].get<xpu, 3, float>();
  seg_op::SegMaxPoolBackwardImpl(dst, ograd, pool_indices, req[0], ctx, s);
}
}  // namespace op
}  // namespace mxnet
